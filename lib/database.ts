...
export async function getDailyLogs(): Promise<DailyLog[]> {
  try {
    ...
    return (
      logs?.map((log) => {
        // More robust creator name resolution - NEVER use legacy created_by as fallback
        ...
        if (log.created_by_user_id && allProfiles) {
          const profile = allProfiles.find((p) => p.id === log.created_by_user_id)
          if (profile) {
            creatorName = profile.full_name || profile.email || "Unknown User"
            console.log(`Found creator profile for log ${log.id}:`, creatorName)
          } else {
            console.log(`No profile found for user_id ${log.created_by_user_id} in log ${log.id}`)
          }
        } else {
          console.log(`Log ${log.id} has no created_by_user_id`)
        }
...
export async function updateDailyLog(id: string, updates: Partial<DailyLog>): Promise<DailyLog | null> {
  try {
    ...
    delete (updateData as any).created_by // Also remove legacy field from updates

    console.log("Updating daily log with preserved creator:", {
      logId: id,
      originalCreator: originalLog.created_by_user_id,
    ...
    // Verify the creator was preserved
    console.log("Updated log creator verification:", {
      originalCreator: originalLog.created_by_user_id,
      updatedCreator: data.created_by_user_id,
      preserved: originalLog.created_by_user_id === data.created_by_user_id,
    })
